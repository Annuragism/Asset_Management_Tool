...
router.get("/hhome",(req,res)=>{if(req.session.hr==null){res.render('login')}else res.render('hrhome',{hr:req.session.hr})})
//--------------------------------------------------------------------------
        ...
        case 'HR':
          {
            ...
            HR.find({Email_id:email,Password:password},(err,result)=>{
              //console.log(result);
              if(err) throw err;
              else if(result.length!=0)
              {
                ...
                if(status=='1')
                 {
                 res.render('login',{msg:'HR De-Activated, Please Try again',})
                 }
                 else
                 {
                 req.session.hr=email;
                 res.render('hrhome',{hr:req.session.hr})
                 }
...
//hr login ends
router.get('/hr-logout',(req,res)=>{
req.session.destroy();
res.render('login',{success_msg:'HR Logout successfully'})
})
//----------------------------------
      ...
      case 'HR':
        {
          ...
          newhr.save().then((data)=>console.log("new HR created by admin"));
          res.render('adminhome',{success_msg:"New HR Successfully Created..!!!"})
...
router.get("/view_user",(req,res)=>{
    ...
    else
    {
      ...
      else
      {
        ...
        {
          HR.find({Status:'0'},(err,hr)=>{
          //console.log(hr);
          if (err) console.log("error");
          else
          {
          res.render('view_user',{employee1:employee,support:support,manager:manager,hr:hr,admin:req.session.admin})
           }
...
router.get('/admin-documentation',(req,res)=>{
  res.render('admin-documentation',{admin:req.session.admin})
})
//---------------------------------------------------------------------------
//add_document_by_admin
router.get('/add_document_by_admin',(req,res)=>{
  res.render('add_document_by_admin',{admin:req.session.admin})
})
//---------------------------------------------------------------------------
//-------------------------------------------------------------------------
//upload files
...
router.post('/add-doc-by-admin-form-action',(req,res)=>{
  if (req.session.admin==null){res.render('login'),{msg:"Session Expired"}}
  ...
  else if(req.files)
  {
    ...
    var uploaded_by=req.session.admin;
    ...
    var newfilename=filename+Math.floor(Math.random()*100);
     file.mv('./uploaded_file/'+newfilename,(err,result)=>{
       if(err) throw err;
       else if(p=='employee') {
           var newfile=File({
             doc:doc,
           ...
           newfile.save().then((data)=>{
             res.render('add_document_by_admin',{success_msg:'File Uploaded and sent to employee',admin:req.session.admin})
           })
       }//employee else if end here
           ...
           newfile.save().then((data)=>{
             res.render('add_document_by_admin',{success_msg:'File Uploaded and sent to hr',admin:req.session.admin})
           })
       }//hr else if end here
           ...
           newfile.save().then((data)=>{
             res.render('add_document_by_admin',{success_msg:'File Uploaded and sent to support',admin:req.session.admin})
           })
       }//support else if end here
           ...
           newfile.save().then((data)=>{
             res.render('add_document_by_admin',{success_msg:'File Uploaded and sent to manager',admin:req.session.admin})
           })
       }//manager else if end here
     })//file mv end here
}
else {
  res.render('add_document_by_admin',{msg:"Somthing went wrong",admin:req.session.admin})
}
})
//---------------------------------------------------------------------------
//add-doc-by-admin-form-action
router.get('/view_document_status_by_admin',(req,res)=>{
  File.find({uploaded_by:req.session.admin},(err,result)=>{
    console.log(result);
  if (err) throw err;
  else if(req.session.admin==null){res.render('login'),{msg:"session Expired"}}
    else if (result.length!=0)
    {
        var requestStatus=result.map((rec)=>{
           ...
           else if(rec.status==110)
           return "Deliverd to HR";
           else if(rec.status==2)
           return "Deliverd to HR, But Pending with Manager";
           else if(rec.status==21)
           return "Rejected by Manager";
           else if(rec.status==210)
           return "Deliverd to Manager";
           else if(rec.status==3)
           return "Deliverd to Manager, But Pending with Support";
           else if(rec.status==31)
           return "Rejected by Support";
           else if(rec.status==310)
           return "Deliverd to Support";
        ...
        res.render('view_document_status_by_admin',{file_data:result,finaldata:finaldata,admin:req.session.admin,hi:"hello"})

    }


  else
  {res.render('view_document_status_by_admin',{msg:"Not able to view the status of documentation",admin:req.session.admin})}
